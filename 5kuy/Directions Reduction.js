...
[West]
Другие примеры:
В ["NORTH", "SOUTH", "EAST", "WEST"]направлении "NORTH" + "SOUTH"идет на север и сразу же возвращается обратно .

Путь становится ["EAST", "WEST"], now "EAST"и "WEST"уничтожают друг друга, поэтому конечный результат равен [](ноль в Clojure).

В ["СЕВЕР", "ВОСТОК", "ЗАПАД", "ЮГ", "ЗАПАД", "ЗАПАД"], "СЕВЕР" и "ЮГ" не являются прямо противоположными, но становятся прямо противоположными после сокращения слова "ВОСТОК" и "WEST", поэтому весь путь можно свести к ["WEST", "WEST"].
...
Примечания
Не все пути можно сделать проще. Путь [«СЕВЕР», «ЗАПАД», «ЮГ», «ВОСТОК»] не сокращается. «СЕВЕР» и «ЗАПАД», «ЗАПАД» и «ЮГ», «ЮГ» и «ВОСТОК» не являются прямо противоположными друг другу и не могут стать таковыми. Следовательно, путь результата сам по себе следующий: ["СЕВЕР", "ЗАПАД", "ЮГ", "ВОСТОК"].
...
function dirReduc(arr) {
    ...
    return arr.reduce((directions, dir) => {
        if (directions[directions.length - 1] === opposite[dir]) {
            directions.pop();
        } else {
            directions.push(dir);
        }
